...
export function Reserves() {  
    ...
    const [dateTarget, setDateTarget] = useState(structuringDate(dateToday));
    const [scheduleTarget, setScheduleTarget] = useState(0)
    const [schedule, setSchedule] = useState("");
    const [collapse, setCollapse] = useState("");
    const [loading, setLoading] = useState(false);
    const [reserves, setReserves] = useState<ReservesProps[]>([])

   useEffect(() => {
        async function getMyReserves() {
            try{
                const reservasResponse = await api.get(`/reservasjoin/${authContext.user?.id}`)

                setReserves(reservasResponse.data)
   ...
   function renderizeReserves() {
       let aux: ReactNode[] = [];

       for(let i = 0; i < reserves.length; i++) {
           aux.push(<MyReserve id={reserves[i].id} src={gym} place={rules[reserves[i].espacos_id-1].nameLocal} data={reserves[i].data} schedule={reserves[i].horario}/>)
       }
    ...
    async function handleCreateReserve(e: FormEvent) {
        ...
        try{
            await api.post('/reservas', {
                data: dateTarget,
                horario: scheduleTarget,
                espacos_id: placeTarget,
